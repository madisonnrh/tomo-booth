...
var bwFilter = document.getElementById("bw-filter");
...
function setSelectedFilter(filter) {
    ...
    filmGrain.classList.toggle("active", isBlackAndWhite);
    colorFilter.classList.toggle("active", !isBlackAndWhite);
    bwFilter.classList.toggle("active", isBlackAndWhite);
    colorFilter.setAttribute("aria-pressed", String(!isBlackAndWhite));
    bwFilter.setAttribute("aria-pressed", String(isBlackAndWhite));
}
...
function beginSession() {
    ...
    startCamera().then(function (cameraStarted) {
        ...
        setMessage(
            setupHelp,
            "Your camera is ready. Choose a filter, then press the button when you are ready."
        );
